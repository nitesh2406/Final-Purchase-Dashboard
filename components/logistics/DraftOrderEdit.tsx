            ...
            <div className="flex justify-between items-center px-1 mb-2">
                <div>
                    <h2 className="text-xl font-semibold flex items-center gap-3">
                        {isCreateMode ? 'Create New Draft PO' : (isViewOnly ? `View Draft PO: ${draft?.id}` : `Edit Draft PO: ${draft?.id}`)}
                    </h2>
                </div>
                <div className="flex items-center gap-3">
                    ...
                    {!isViewOnly && (
                        <>
                            ...
                            {!isCreateMode && <Button onClick={handleSubmitClick} disabled={isSaving || isSubmitting} className="h-10 px-6 bg-indigo-600 hover:bg-indigo-700 font-bold shadow-lg shadow-indigo-900/20">{isSaving ? 'Saving...' : 'Submit Order'}</Button>}
                        </>
                    )}
                </div>
            </div>

            <div className={`bg-slate-800 rounded-lg p-6 mb-2 shadow-sm border border-slate-700/50 ${isViewOnly ? 'opacity-80' : ''}`}>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-6 mb-6">
                    <div>
                        <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1.5">PO Date</label>
                        <input
                            type="date"
                            value={poDate}
                    ...
                    <div>
                        <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1.5">Expected Delivery</label>
                        <input
                            type="date"
                    ...
                    <div>
                        <label className="block text-xs font-bold text-slate-500 uppercase tracking-widest mb-1.5">Shipping Mode *</label>
                        <select
                            value={shippingMode}
                                                        ...
                                                        >
                                                            ...
                                                            {vendorMasters.map(vm => {
                                                                ...
                                                                return (
                                                                    ...
                                                                    >
                                                                        {isSubmitted ? `🔒 ${vm.vendor_name} (Submitted)` : vm.vendor_name}
                                                                    </option>
                                                                );
                                                            })}
                                                        </select>
