...
import { ArrowsUpDownIcon, ArrowPathIcon, ShipIcon, AirplaneIcon, ClockIcon, PencilIcon, ExclamationTriangleIcon, ArrowPathIcon as RetryIcon } from '../icons/Icons';
...
export const InventoryForecasting: FC<InventoryForecastingProps> = ({ isSidebarCollapsed }) => {
    ...
    const handleCreateDraft = async () => {
        ...
        try {
            ...
            if (result.draftId) {
                alert(`Draft Order ${result.draftId} created successfully from forecast.`);
                setSelectedSkuIds([]); // Clear selection after success
             ...
             <table className="min-w-full divide-y-2 divide-transparent border-spacing-y-2">
                <thead className="bg-gray-50 dark:bg-gray-700 sticky top-0 z-10">
                    <tr>
                        ...
                        <SortableHeader label="In Stock" sortKey="inStock" className="text-center" />
                ...
                <tbody className="bg-white dark:bg-slate-900">
                    {sortedSkus.map(sku => (
                        <tr key={sku.masterSKU} onClick={() => handleRowClick(sku)} className={`cursor-pointer group border-b border-slate-700/50 odd:bg-slate-900 even:bg-slate-800/50 ${selectedSkuIds.includes(sku.masterSKU) ? 'bg-primary-900/30' : 'hover:bg-slate-700/50'}`}>
                            ...
                            <td className="px-3 py-3 whitespace-nowrap text-base text-white text-center">{sku.monthlyMovingAvg.toFixed(1)}</td>
                            ...
                            <td className="px-3 py-3 whitespace-nowrap text-center">
                                <div className={`text-base font-semibold ${getDaysOfCoverColor(sku.daysOfCover)}`}>
            ...
            <div className="flex flex-wrap justify-between items-center gap-4">
                <div className="flex items-center gap-4 flex-grow md:flex-grow-0 md:w-1/3">
                    ...
                    <button 
                        ...
                        className={`px-2 py-1 rounded text-[9px] font-bold uppercase tracking-widest transition-all border shrink-0 ${
                            showDebug 
                     ...
                     <div className="flex items-center gap-2 flex-wrap">
                        ...
                        {(['All', 'Urgent', 'Low Stock', 'Healthy', 'Awaiting Inbound', 'Stockout Gap'] as const).map(s => (
                            <Button 
                                ...
                                className={`text-xs py-1 px-3 capitalize ${
                                    s === 'Stockout Gap' 
                                    ? (statusFilter === s 
