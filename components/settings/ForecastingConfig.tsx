    ...
    return (
        <div className="space-y-6 pb-12">
            ...
            <div className="flex items-start justify-between">
                <div>
                    <h2 className="text-base font-bold text-white">Forecasting Configuration</h2>
                ...
                >
                    ...
                    <div className="mt-4 bg-slate-900/60 border border-slate-700/40 rounded-lg p-3 font-mono text-[11px] leading-relaxed">
                        <div className="flex items-center gap-2">
                            ...
                            <span className="text-slate-300">if Cost &gt; ₹{config.MIN_COST_AIR} OR Monthly Avg &lt; {config.LOW_MMA_AIR} units</span>
                        </div>
                        <div className="flex items-center gap-2 mt-1">
                            <span className="text-blue-400 font-bold">SEA</span>
                            <span className="text-slate-500">→</span>
                            <span className="text-slate-300">if Cost ≤ ₹{config.MIN_COST_AIR} AND Monthly Avg ≥ {config.LOW_MMA_AIR} units</span>
                        </div>
                    </div>
                </SectionCard>
            </div>
    ...
    <div className="flex items-start justify-between border-b border-slate-700/50 last:border-0 py-4">
        <div className="max-w-[70%]">
            <div className="flex items-center gap-2">
                ...
                {tag && (
                    <span className="px-1.5 py-0.5 rounded bg-slate-700 text-slate-400 text-[8px] font-bold uppercase tracking-tighter">
                        {tag}
                    </span>
                )}
            </div>
...
const WeightInput: React.FC<WeightInputProps> = ({ label, hint, value, onChange }) => (
    <div className="bg-slate-900/40 border border-slate-700/40 rounded-lg p-3 flex items-center justify-between">
        <div>
        ...
        <div className="flex items-center gap-2">
            <input 
                ...
                max={1}
                onChange={(e) => onChange(parseFloat(e.target.value) || 0)}
                className="w-16 text-center text-xs font-bold text-blue-300 bg-slate-800 border border-slate-700 rounded px-1 py-1 focus:ring-1 focus:ring-blue-500 outline-none"
            />
            <span className="text-[9px] font-mono text-slate-500 w-8">({Math.round(value * 100)}%)</span>
        </div>
    </div>
);
