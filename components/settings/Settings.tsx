...
export const Settings: React.FC<{
    ...
    return (
        <div className="flex flex-col md:flex-row gap-8 h-full p-6 bg-white dark:bg-slate-900 overflow-hidden">
            <aside className="w-full md:w-1/4 lg:w-1/5 overflow-y-auto">
                <h2 className="text-lg font-bold mb-6 text-slate-800 dark:text-white tracking-tight">Settings</h2>
                    ...
                    {settingsNav.map(item => (
                        <button
                            ...
                            className={`w-full flex flex-col items-start gap-1 px-4 py-3 text-sm font-medium rounded-xl transition-all duration-200 border group ${
                                ...
                                ? 'bg-blue-500/10 text-blue-600 dark:text-blue-400 border-blue-500/20 dark:border-blue-500/30 shadow-sm'
                                : 'text-slate-500 dark:text-slate-400 border-transparent hover:bg-slate-200/50 dark:hover:bg-slate-800/60 hover:text-slate-800 dark:hover:text-slate-200'
                            }`}
                        >
                            <div className="flex items-center gap-3 w-full">
                                <div className={`transition-colors ${activeView === item.name ? 'text-blue-600 dark:text-blue-400' : 'text-slate-400 dark:text-slate-500 group-hover:text-slate-600 dark:group-hover:text-slate-300'}`}>
                                    {item.icon}
                                </div>
                                <span className="flex-1 text-left">{item.name}</span>
                                {item.isLive && (
                                    <span className="px-1.5 py-0.5 rounded-full bg-blue-500/10 text-blue-600 dark:text-blue-400 text-[8px] font-black uppercase tracking-widest border border-blue-500/20">
                                        Live
                                    </span>
                                )}
                            </div>
                            <span className={`text-[10px] ml-8 font-normal transition-opacity duration-200 ${
                                activeView === item.name ? 'text-slate-500 dark:text-slate-400 opacity-100' : 'text-slate-400 dark:text-slate-600 opacity-0 group-hover:opacity-100'
                            }`}>
                                {item.description}
                            </span>
                        </button>
